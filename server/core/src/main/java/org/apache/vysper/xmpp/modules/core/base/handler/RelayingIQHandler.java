...
/**
 * most IQ are targeted to the server. but sometimes, IQs are only routed
 * through the server to another client. this is the handler dealing with that.
 */
public class RelayingIQHandler extends IQHandler {

	private static final Logger logger = LoggerFactory.getLogger(RelayingIQHandler.class);
	...
	protected Stanza executeIQLogic(IQStanza stanza, ServerRuntimeContext serverRuntimeContext, boolean outboundStanza,
		...
		// in the case where an IQ is send to the server,
		// StanzaHandlerLookup.getIQHandler is responsible for
